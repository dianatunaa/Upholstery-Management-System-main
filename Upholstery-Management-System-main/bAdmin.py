...
tab4 = Frame(tab_parent, bg = '#7fcbab')

pic_monitor = PhotoImage(file = r'D:\Users\Diane\Desktop\Upholstery-Management-System-main\monitor.png')
pic_inventory = PhotoImage(file = r'D:\Users\Diane\Desktop\Upholstery-Management-System-main\monitor.png')
pic_customer = PhotoImage(file = r'D:\Users\Diane\Desktop\Upholstery-Management-System-main\cust.png')
pic_account = PhotoImage(file = r'D:\Users\Diane\Desktop\Upholstery-Management-System-main\account.png')
...
tab_parent.add(tab4, text = "Manage Accounts", image = pic_account, compound = LEFT)
...
def AccDisplay():
    ...
    fetch = curr.fetchall()
    for data in fetch:
        acc_tree.insert('', 'end', values=(data))
    curr.close()
    conn.close()
...
def AccAdd():
    ...
    add_acc.geometry("350x300")
    add_acc.configure(bg = '#7fcbab')
    add_acc.resizable(False, False)
    add_acc.lift()
    add_acc.attributes("-topmost", True)
    add_acc.grab_set()

    title_frame = Frame(add_acc, bg = '#ffffff', width = 600)
    title_frame.pack(side = TOP, pady = 15, fill = X)
    lbl_text = Label(title_frame, bg = '#ffffff', text="Add New Account", font = ("Consolas", 12))
    lbl_text.pack(fill = Y)
    add_frame = Frame(add_acc, bg = '#7fcbab')
    add_frame.pack(side = TOP, padx = 10, pady = 12)
    bot_frame = Frame(add_acc, bg = '#7fcbab')
    bot_frame.pack(side = TOP, padx = 10, pady = 12)
    employee.set("")
    username.set("")
    password.set("")
    authority.set("")
    
    lbl_employee = Label(add_frame, bg = '#7fcbab', text = "Employee Name:", font = ("Consolas", 10), bd = 5)
    lbl_employee.grid(row = 0, sticky = W)
    lbl_prodID = Label(add_frame, bg = '#7fcbab', text = "Username:", font = ("Consolas", 10), bd = 5)
    lbl_prodID.grid(row = 1, sticky = W)
    lbl_prodname = Label(add_frame, bg = '#7fcbab', text = "Password:", font = ("Consolas", 10), bd = 5)
    lbl_prodname.grid(row = 2, sticky = W)
    lbl_mat = Label(add_frame, bg = '#7fcbab', text = "Authorization:", font = ("Consolas", 10), bd = 5)
    lbl_mat.grid(row = 3, sticky = W)
    
    ent_employee = Entry(add_frame, textvariable = employee, width = 30)
    ent_employee.grid(row = 0, column = 1, pady = 5, sticky = W)
    ent_username = Entry(add_frame, textvariable = username, width = 30)
    ent_username.grid(row = 1, column = 1, pady = 5, sticky = W)
    ent_password = Entry(add_frame, textvariable = password, width = 30, show = '*')
    ent_password.grid(row = 2, column = 1, pady = 5, sticky = W)
    ent_authorization = ttk.Combobox(add_frame, width = 27, textvariable = authority, state = 'readonly')
    ent_authorization['values'] = ("Administrator", "Project Coordinator", "Inventory Clerk")
    ent_authorization.grid(row = 3, column = 1, sticky = W)

    btn_cancel = Button(bot_frame, text = "Cancel", width = 15, font = ('Consolas', 10), bg = '#b7e2d0', relief = FLAT, overrelief = SUNKEN, command = add_acc.destroy)
    btn_cancel.grid(row = 0, column = 0)
    btn_add = Button(bot_frame, text = "Save", width = 15, font = ('Consolas', 10), bg = '#b7e2d0', relief = FLAT, overrelief = SUNKEN, command = AccAddValidate)
    btn_add.grid(row = 0, column = 1, pady = 10, padx = 10)

def AccAddValidate():
    dbFunc()
    curr.execute("""SELECT username FROM account""")
    fetch = curr.fetchall()
    usernames = [x[0] for x in fetch]
    if employee.get() == "" or username.get() == "" or password.get() == ""or authority.get() == "":
        error = tkMessageBox.showerror('Error', 'Invalid, Fill all entries')
    elif username.get() in usernames:
        error = tkMessageBox.showerror('Error', 'Invalid, Username already exist')
    else:   
        curr.execute("""INSERT INTO account VALUES(?, ?, ?, ?)""", (str(employee.get()), str(username.get()), str(password.get()), str(authority.get())))
        conn.commit()
        curr.close()
        conn.close()
        add_acc.grab_release()
        add_acc.destroy()
        AccRefresh()

def AccEdit():
    if not acc_tree.selection():
       print("ERROR")
    else:
        global edit_acc
        edit_acc = Toplevel()
        edit_acc.title("Edit Account")
        edit_acc.geometry("350x300")
        edit_acc.configure(bg = '#7fcbab')
        edit_acc.resizable(False, False)
        edit_acc.lift()
        edit_acc.attributes("-topmost", True)
        edit_acc.grab_set()

        title_frame = Frame(edit_acc, bg = '#ffffff', width = 600)
        title_frame.pack(side = TOP, padx = 10, pady = 10, fill = X)
        add_frame = Frame(edit_acc, bg = '#7fcbab')
        add_frame.pack(side = TOP, padx = 10, pady = 12)
        bot_frame = Frame(edit_acc, bg = '#7fcbab')
        bot_frame.pack(side = TOP, padx = 10, pady = 12)        
        lbl_text = Label(title_frame, bg = '#ffffff', text = "Edit Account", font = ("Consolas", 12))
        lbl_text.pack(padx = 10)

        curItem = acc_tree.focus()
        contents =(acc_tree.item(curItem))
        selecteditem = contents['values']
        employee.set(selecteditem[0])
        username.set(selecteditem[1])
        password.set(selecteditem[2])
        authority.set(selecteditem[3])
        
        lbl_distinct = Label(add_frame, text = "Employee Name:", bg = '#7fcbab', font = ("Consolas", 10), bd = 5)
        lbl_distinct.grid(row = 0, sticky = W)
        lbl_prodID = Label(add_frame, text = "Username:", bg = '#7fcbab', font = ("Consolas", 10), bd = 5)
        lbl_prodID.grid(row = 1, sticky = W)
        lbl_prodname = Label(add_frame, text = "Password:", bg = '#7fcbab', font = ("Consolas", 10), bd = 5)
        lbl_prodname.grid(row = 2, sticky = W)
        lbl_mat = Label(add_frame, text = "Authorization:", bg = '#7fcbab', font = ("Consolas", 10), bd = 5)
        lbl_mat.grid(row = 3, sticky = W)

        ent_employee = Label(add_frame, text = employee.get(), bg = '#7fcbab', font = ("Consolas", 10))
        ent_employee.grid(row = 0, column = 1, pady = 5, sticky = W)
        ent_username = Entry(add_frame, textvariable = username, width = 25)
        ent_username.grid(row = 1, column = 1, pady = 5, sticky = W)
        ent_password = Entry(add_frame, textvariable = password, width = 25)
        ent_password.grid(row = 2, column = 1, pady = 5, sticky = W)
        ent_authorization = ttk.Combobox(add_frame, width = 22, textvariable = authority, state = 'readonly')
        ent_authorization['values'] = ("Administrator", "Project Coordinator", "Inventory Clerk")
        ent_authorization.grid(row = 3, column = 1, sticky = W)

        btn_cancel = Button(bot_frame, text = "Cancel", width = 15, font = ('Consolas', 10), bg = '#b7e2d0', relief = FLAT, overrelief = SUNKEN, command = edit_acc.destroy)
        btn_cancel.grid(row = 0, column = 0)
        btn_add = Button(bot_frame, text = "Save", width = 15, font = ('Consolas', 10), bg = '#b7e2d0', relief = FLAT, overrelief = SUNKEN, command = AccEditValidate)
        btn_add.grid(row = 0, column = 1, pady = 10, padx = 10)
...
def AccEditValidate():
    ...
    if employee.get() == "" or username.get() == "" or password.get() == ""or authority.get() == "":
        ...
        contents =(inventory_tree.item(curItem))
        selecteditem = contents['values']
        curr.execute("""UPDATE account SET username = ?, password = ?, authority = ?  WHERE employee = ?""", (str(username.get()), str(password.get()), str(authority.get()), str(employee.get())))
        conn.commit()
        curr.close()
        conn.close()
        AccRefresh()
...
def AccDelete():
    if not acc_tree.selection():
       print("ERROR")
        ...
        result = tkMessageBox.askquestion('Delete', 'Delete Account?', icon="warning")
        if result == 'yes':
            curItem = acc_tree.focus()
            contents =(acc_tree.item(curItem))
            selecteditem = contents['values']
            acc_tree.delete(curItem)
            dbFunc()
            curr.execute("""DELETE FROM account WHERE employee = ?""", (selecteditem[0], ))
            conn.commit()
            curr.close()
            conn.close()
...
##########################Manage Accounts-tab4######################
acc_left_frame = Frame(tab4, width=600, bg = '#7fcbab')
acc_left_frame.pack(side=LEFT, fill=Y)
acc_tree_frame = Frame(tab4, width = 600, bg = '#7fcbab')
acc_tree_frame.pack(side = TOP)

acc_btn_reset = Button(acc_left_frame, text = "Refresh", width = 15, font = ('Consolas', 10), bg = '#b7e2d0', relief = FLAT, overrelief = SUNKEN, command = AccRefresh)
acc_btn_reset.pack(side = TOP, padx = 10, pady = 10, fill = X)
acc_btn_add = Button(acc_left_frame, text = "Add", width = 15, font = ('Consolas', 10), bg = '#b7e2d0', relief = FLAT, overrelief = SUNKEN, command = AccAdd)
acc_btn_add.pack(side = TOP, padx = 10, pady = 10, fill = X)
acc_btn_edit = Button(acc_left_frame, text = "Edit", width = 15, font = ('Consolas', 10), bg = '#b7e2d0', relief = FLAT, overrelief = SUNKEN, command = AccEdit)
acc_btn_edit.pack(side = TOP, padx = 10, pady = 10, fill = X)
acc_btn_delete = Button(acc_left_frame, text = "Delete", width = 15, font = ('Consolas', 10), bg = '#b7e2d0', relief = FLAT, overrelief = SUNKEN, command = AccDelete)
acc_btn_delete.pack(side = TOP, padx = 10, pady = 10, fill = X)

acc_scrollbarx = Scrollbar(acc_tree_frame, orient = HORIZONTAL)
acc_scrollbary = Scrollbar(acc_tree_frame, orient = VERTICAL)
acc_tree = ttk.Treeview(acc_tree_frame, columns = ('Employee', 'Username', 'Password', 'Authorization'), selectmode = "extended", height = 100, yscrollcommand = acc_scrollbary.set, xscrollcommand = acc_scrollbarx.set)
acc_scrollbary.config(command = acc_tree.yview)
acc_scrollbary.pack(side = RIGHT, fill = Y)
acc_scrollbarx.config(command = acc_tree.xview)
acc_scrollbarx.pack(side = BOTTOM, fill = X)
acc_tree.heading('Employee', text = "Employee Name",anchor = W)
acc_tree.heading('Username', text = "Username",anchor = W)
acc_tree.heading('Password', text = "Password",anchor = W)
acc_tree.heading('Authorization', text = "Authorization",anchor = W)
acc_tree.column('#0', stretch = NO, minwidth = 0, width = 0)
acc_tree.column('#1', width=210)
acc_tree.column('#2', width=210)
acc_tree.column('#3', width=210)
acc_tree.column('#4', width=210)
acc_tree.pack()
AccDisplay()
